import { collection, addDoc, serverTimestamp, getDoc, doc, query, where, or, getDocs, Timestamp, updateDoc, arrayUnion } from 'firebase/firestore';
import '../css/home.css'
...
export default function Home(){
        ...
        const dataArr = await Promise.all(
            snapshots.docs.map(async(v) => {
                ...
                if(!!comments.length){
                    ...
                    return ({...v.data(), user_name : data2.general.name, user_id : data2.general.id , user_photo : data2.general.photoURL, time : timeObj, userInfo : userData1, postID : v.id});
                }
                return ({...v.data(), user_name : data2.general.name, user_id : data2.general.id , user_photo : data2.general.photoURL, time : timeObj, postID : v.id});

            })
        )
        const dataArr2 = [...dataArr].sort((a,b) => a.time.seconds - b.time.seconds).reverse()
    ...
    function CommentComponent({data}){
        ...
        if(!!data){
            ...
            return(
                <div className={`comment_${data.postID}`}>
                    ...
                    <div className="h_f_com_lists">
                        <ul>
                            { !data.comment.length? (
                                ...
                                data.comment.map((val,idx) => {return(
                                    <li className="h_f_com_unit" key={"comment_"+idx}>
                                        <div className="h_f_com_u_account">
                                            <div className="h_f_com_u_avatar" onClick={(e) => navigate(`/account/${val.uid}`)}>
                                                <img src={data.userInfo.find(v => v.uid === val.uid).photoURL} />
                                            </div>
                                            <div className="h_f_com_u_name">@{data.userInfo.find(v => v.uid === val.uid).id}</div>
                                            <div className="h_f_com_u_time">{data.time.seconds}</div>
                                        </div>
                                        <div className="h_f_com_u_text">{data.text}</div>
                                    </li>
                                )})
                            )
                    ...
                    <div className="h_f_com_input">
                        <form onSubmit={(e)=> setComment(e, data.postID)}>
                            <input ref={commentUnit} type="text" name='text'/>
    ...
    const CommentMemo = useMemo(CommentComponent)
                                ...
                                totalFeed.map((v,i) => (
                                    <li className='h_f_list' key={v['user_id'] + "_" + i}>
                                        ...
                                        <div className="h_f_comment">
                                            ...
                                            <div className="h_f_com_input">
                                                <form onSubmit={(e)=> setComment(e,v.postID)}>
                                                    <input type="text" name='text'/>
                                                    <button>submit</button>
                                                </form>
                                            </div> */}
                                            <CommentMemo id={`comments_${v.postID}`} data={v}/>
                                        </div>
                                    </li>
                                ))
                            )}
